...
The surfaces and their ranks are data, not prose: `data/evidence-surfaces.json`
owns them and `tests/test_evidence_surfaces.py` holds them. This section states
what the ranking is FOR; that file states what is in it.

**THE BOTTOM OF THE RANKING IS NOT ONE PILE.** Read as a single line — community
documentation, blogs, samples and issues — it says only *do not cite this*,
which is correct and is the smaller half of what is true. Two different things
live down there and each names a different way out:

| What it carries | The way out | Where it lands |
|---|---|---|
| An **identifier** — a digest reproducing `MC1319213 \| May 22 – …` | read that identifier at its own address | rank 2, first-party |
| A **method** — somebody ran the thing and wrote down what it did | run it here | rank 1, this engine's own observation |

Neither is ever a basis, and the difference between them is not credibility. It
is which escalation is owed before anything is cited.

The second kind decays and the first does not. A message identifier is fixed and
will read the same in five years; a published experiment was true against the
platform on the day it ran, and the platform is the thing that changes. **An
investigation is never cited for its conclusion**, only for the method, and the
conclusion is re-established by running it.

There is one thing rank 5 can do that rank 2 cannot, and it is why it is in this
file at all: it can reach the reader. The Tenant Admin newsletter arrives in an
administrator's own message centre and nowhere public, so a change can be real,
dated and unfindable by anyone outside the tenant that received it. A digest
that keeps the identifier hands back the address. What must never travel is the
digest's own prose — several of those summaries are generated by a model, and a
paraphrase of a paraphrase is how a product publishes a sentence Microsoft never
wrote.

### The order the surfaces are asked in

The surfaces above say what each one settles. This says what to do with them,
because a list of sources is not a method and `I searched` still is not one.

1. **State the claim as a sentence that can be false.** *Report-only policies
   enforce nothing* can be. *Conditional Access is important* cannot, and no
   amount of reading will settle it.
2. **Ask what KIND of claim it is**, because the kind chooses the surface and
   nothing else does. Documented behaviour → the reference pages. Tenant state →
   the authoritative surface for that state, and the surfaces are not
   interchangeable. Runtime behaviour → an experiment. A population → an
   enumeration, because a search result establishes what the query matched and
   never what the population contains. A commercial claim → the person or the
   independent verifier it requires.
3. **Read the primary surface for that kind, then the change surfaces.** Both,
   before writing. Not because the change surfaces usually say something, but
   because when they do they move what the answer MEANS, and a rule correct on
   the day it was written keeps being published long after it stopped being.
4. **Where two surfaces disagree, record the disagreement.** The response
   outranks the prose: the answer is what is true and the page is what somebody
   intended. Do not resolve it in favour of whichever was read second. The
   divergence is often the finding.
5. **If nothing settles it, escalate the SURFACE, not the confidence.** A rank 5
   result is a route, per the table above, and following it is part of the work
   rather than a next step for somebody else.
6. **Terminate in one of the named states**, which are the six in the table
   under *`not established` is a conclusion, not a way of stopping early* and
   are not restated here. Anything outside that set is a sentence that will be
   read as a fact by whoever reads it next.

**Then write once.** Every rewrite of `SPO-SCRIPT-001` was a surface that had
not been asked, and each one cost more than the reading would have.
